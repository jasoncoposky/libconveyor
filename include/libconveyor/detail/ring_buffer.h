...
struct RingBuffer { // Still needed for read buffer
    ...
    RingBuffer(size_t cap) : capacity(cap), buffer(new char[cap]) {}
    ...
    void resize(size_t new_capacity) {
        ...
        if (size > 0) {
            if (head > tail) {
                ...
                std::memcpy(new_buffer.get(), buffer.get() + tail, size);
            } else {
                // Data is in two parts (wrapped)
                size_t first_chunk = capacity - tail;
                std::memcpy(new_buffer.get(), buffer.get() + tail, first_chunk);
                std::memcpy(new_buffer.get() + first_chunk, buffer.get(), head);
            }
        }
    ...
    size_t write(const char* data, size_t len) {
        ...
        std::memcpy(buffer.get() + head, data, first_chunk_len);

        if (bytes_to_write > first_chunk_len) {
            std::memcpy(buffer.get(), data + first_chunk_len, bytes_to_write - first_chunk_len);
        }
    ...
    size_t read(char* data, size_t len) {
        ...
        if (data != nullptr) {
            ...
            std::memcpy(data, buffer.get() + tail, first_chunk_len);
            if (bytes_to_read > first_chunk_len) {
                std::memcpy(data + first_chunk_len, buffer.get(), bytes_to_read - first_chunk_len);
            }
        }
    ...
    size_t peek_at(size_t absolute_ring_pos, char* dest, size_t len) const {
        ...
        std::memcpy(dest, buffer.get() + offset, first_chunk);
        if (len > first_chunk) {
            std::memcpy(dest + first_chunk, buffer.get(), len - first_chunk);
        }
        return len;
    }
