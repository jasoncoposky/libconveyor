...
template <typename T>
struct is_contiguous<T, std::void_t<decltype(std::data(std::declval<T>())),
...
public:
  ...
  static Result<Conveyor> create(const Config &cfg_v2) {
    ...
    cfg_c.max_write_size =
        cfg_v2.write_capacity;                  // For now, initial size is max
